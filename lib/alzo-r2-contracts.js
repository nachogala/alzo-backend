...
const MIN_VALID_AUDIO_MS = 40000;
...
Use only Goal, Purpose, Reconnection Anchor, Mood, Energy, Alignment as emotional relationship to the 90-day Goal, First Message as continuity reference only and the selected canonical Daily history.

Never use Commitment or Journal. Never interpret Alignment as compliance, productivity, discipline, performance, objective progress, success, failure or task completion. Never invent a fact, plan, action, result or new Goal. Never speak in second person or as ALZO, an app, coach, mentor, narrator, companion, motivational speaker or fictional future self.
...
function validateAuthoritativeSemanticContext(raw = {}, { expectedSha256 = null } = {}) {
  ...
  for (const key of AUTHORITATIVE_SEMANTIC_CONTEXT_KEYS) {
    const item = raw[key];
    if (!item || typeof item !== 'object' || !clean(item.text) || !Array.isArray(item.sourceRefs) || item.sourceRefs.length < 1) {
...
function buildDailyPrompt(context = {}) {
  ...
  const input = {
    ...
    checkIn: {
      ...
      energy: clean(checkIn.energy || context.energy),
    ...
    firstMessageReference: context.firstMessageReference ? {
      ...
      text: clean(context.firstMessageReference.text),
    } : null,
  ...
  return {
    promptVersion: 'alzo.daily.r2.v1',
    semanticContext: semantic,
